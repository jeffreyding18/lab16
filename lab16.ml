...
let go (v : vehicle) (distance : float) (direction : float) : vehicle =
    match v with
    | Bus (p, e) -> Bus (offset p distance direction, e -. (distance /. (get_efficiency v)))
    | Car (p, e) -> Car (offset p distance direction, e -. (distance /. (get_efficiency v)))
    | Truck (p, e) -> Truck (offset p distance direction, e -. (distance /. (get_efficiency v)))
    ;;
  ...
  object
    ...
    method go (distance : float) (direction : float) : unit =
        if energy *. efficiency > distance then
            ...
            (pos <- offset pos (energy *. efficiency) direction ;
            energy <- 0.;
...
class bus (initial_energy : float) (initial_pos : point) (seats : int) =
  object (this)
    ...
    method pick_up (new_passengers : int) : unit =
        if passengers + new_passengers <= seats then
            passengers <- passengers + new_passengers
        else
            passengers <- seats

    method drop_off (num_passengers : int) : unit =
        if passengers - num_passengers >= 0 then
